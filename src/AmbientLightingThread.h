...
private:
  ...
  Adafruit_NeoPixel pixels =
      Adafruit_NeoPixel(NEOPIXEL_COUNT, NEOPIXEL_DATA, NEOPIXEL_TYPE);
#endif
#endif

public:
  explicit AmbientLightingThread(ScanI2C::DeviceType type)
      : OSThread("AmbientLighting") {
    ...
    moduleConfig.ambient_lighting.red =
        (myNodeInfo.my_node_num & 0xFF0000) >> 16;
    ...
    if (_type == ScanI2C::NCP5623) {
      ...
      if (_type == ScanI2C::LP5562) {
        ...
        setLighting(moduleConfig.ambient_lighting.current,
                    moduleConfig.ambient_lighting.red,
                    moduleConfig.ambient_lighting.green,
...
#ifdef HAS_NEOPIXEL_ANIMATION
    static constexpr uint8_t animationEffectCount() { return EFFECT_COUNT; }
    ...
    bool isEnabled() const {
      return moduleConfig.ambient_lighting.led_state &&
             moduleConfig.ambient_lighting.current > 0;
    }

    uint8_t nextEffect() {
      const uint8_t effect =
          (moduleConfig.ambient_lighting.animation + 1) % EFFECT_COUNT;
      setEffect(effect);
      return effect;
    }
    ...
    uint8_t previousEffect() {
      const uint8_t effect =
          (moduleConfig.ambient_lighting.animation + EFFECT_COUNT - 1) %
          EFFECT_COUNT;
      setEffect(effect);
      return effect;
    }
    ...
    void showAlert() {
      ...
      pixels.setColor(moduleConfig.ambient_lighting.red,
                      moduleConfig.ambient_lighting.green,
                      moduleConfig.ambient_lighting.blue);
      pixels.start();
  ...
  protected:
...
#ifdef HAS_NEOPIXEL_ANIMATION
      if (overrideMode != OverrideMode::NONE) {
        const uint32_t duration = overrideMode == OverrideMode::PAIRING
                                      ? PAIRING_DURATION_MS
                                      : ALERT_DURATION_MS;
...
#ifdef HAS_RGB_LED
#if defined(HAS_NCP5623) || defined(HAS_LP5562)
    if ((_type == ScanI2C::NCP5623 || _type == ScanI2C::LP5562) &&
        moduleConfig.ambient_lighting.led_state) {
#endif
      setLighting(moduleConfig.ambient_lighting.current,
                  moduleConfig.ambient_lighting.red,
                  moduleConfig.ambient_lighting.green,
    ...
    CallbackObserver<AmbientLightingThread, void *> notifyDeepSleepObserver =
        CallbackObserver<AmbientLightingThread, void *>(
            this, &AmbientLightingThread::setLightingOff);
