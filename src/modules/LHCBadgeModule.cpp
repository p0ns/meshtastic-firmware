...
bool getArgument(const std::string &command, const char *longName, char alias,
                 std::string &argument) {
  const size_t longLength = strlen(longName);
  if (command.compare(0, longLength, longName) == 0 &&
      (command.size() == longLength ||
       std::isspace(static_cast<unsigned char>(command[longLength])))) {
...
bool parseInteger(const std::string &text, int &value) {
  ...
  if (end == text.c_str() || *end != '\0' || parsed < INT_MIN ||
      parsed > INT_MAX) {
    return false;
  }
...
LHCBadgeModule::LHCBadgeModule()
    : SinglePortModule("lhc-badge", meshtastic_PortNum_TEXT_MESSAGE_APP) {
...
bool LHCBadgeModule::isConfigChannel(ChannelIndex channel) {
  return channel < channels.getNumChannels() &&
         strcmp(channels.getByIndex(channel).settings.name,
                CONFIG_CHANNEL_NAME) == 0;
}

bool LHCBadgeModule::isLhcChannel(ChannelIndex channel) {
  return channel < channels.getNumChannels() &&
         strcmp(channels.getByIndex(channel).settings.name, LHC_CHANNEL_NAME) ==
             0;
}

bool LHCBadgeModule::handleLocalConfigPacket(meshtastic_MeshPacket &packet) {
  if (!isConfigChannel(packet.channel) ||
  ...
  const std::string command(
      reinterpret_cast<const char *>(packet.decoded.payload.bytes),
...
ProcessMessage
LHCBadgeModule::handleReceived(const meshtastic_MeshPacket &packet) {
  if (isLhcChannel(packet.channel) && packet.decoded.payload.size >= 5 &&
      memcmp(packet.decoded.payload.bytes, "disco", 5) == 0 &&
...
std::string LHCBadgeModule::executeCommand(const std::string &command) {
  ...
  if (getArgument(command, "/effect", 'e', argument)) {
    if (!parseInteger(argument, value) ||
        !ambientLightingThread->setEffect(value)) {
  ...
  if (getArgument(command, "/speed", 's', argument)) {
    if (!parseInteger(argument, value) ||
        !ambientLightingThread->setSpeed(value)) {
  ...
  if (getArgument(command, "/color", 'c', argument)) {
    ...
    int blue = -1;
    std::string extra;
    if (!(values >> red >> green >> blue) || values >> extra || red < 0 ||
        red > 255 || green < 0 || green > 255 || blue < 0 || blue > 255) {
      return "Invalid RGB format. Usage: /color <R> <G> <B>";
    }
    ambientLightingThread->setColor(static_cast<uint8_t>(red),
  ...
  if (command == "/show" || command == "d") {
    ...
    settings << "Current settings:\n"
             << "Status: "
             << (ambientLightingThread->isEnabled() ? "ON" : "OFF") << "\n"
             << "Effect: "
             << static_cast<int>(moduleConfig.ambient_lighting.animation)
             << "\n"
             << "Brightness: "
             << static_cast<int>(moduleConfig.ambient_lighting.brightness)
             << "\n"
             << "Speed: " << moduleConfig.ambient_lighting.speed << "\n"
             << "Color: R="
             << static_cast<int>(moduleConfig.ambient_lighting.red)
...
void LHCBadgeModule::sendResponse(const meshtastic_MeshPacket &request,
                                  const std::string &response) {
  const size_t totalParts = std::max<size_t>(
      1, (response.size() + RESPONSE_CHUNK_SIZE - 1) / RESPONSE_CHUNK_SIZE);
  for (size_t part = 0; part < totalParts; ++part) {
    std::string chunk =
        response.substr(part * RESPONSE_CHUNK_SIZE, RESPONSE_CHUNK_SIZE);
    if (totalParts > 1) {
      chunk = "[" + std::to_string(part + 1) + "/" +
              std::to_string(totalParts) + "] " + chunk;
    }
    ...
    packet->decoded.payload.size =
        std::min(chunk.size(), sizeof(packet->decoded.payload.bytes));
    memcpy(packet->decoded.payload.bytes, chunk.data(),
           packet->decoded.payload.size);
...
int LHCBadgeModule::onBluetoothStatusUpdate(const meshtastic::Status *status) {
  ...
  const auto *bluetooth =
      static_cast<const meshtastic::BluetoothStatus *>(status);
  if (bluetooth->getConnectionState() ==
      meshtastic::BluetoothStatus::ConnectionState::PAIRING) {
...
int LHCBadgeModule::onInputEvent(const InputEvent *event) {
  if (!event || strcmp(event->source, "LHCBadgeButton") != 0 ||
      !ambientLightingThread) {
    return 0;
  }
