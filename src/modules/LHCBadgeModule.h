...
class LHCBadgeModule : public SinglePortModule
{
  ...
  private:
    CallbackObserver<LHCBadgeModule, const meshtastic::Status *> bluetoothStatusObserver =
        CallbackObserver<LHCBadgeModule, const meshtastic::Status *>(this, &LHCBadgeModule::onBluetoothStatusUpdate);
    CallbackObserver<LHCBadgeModule, const InputEvent *> inputObserver =
        CallbackObserver<LHCBadgeModule, const InputEvent *>(this, &LHCBadgeModule::onInputEvent);

    int onBluetoothStatusUpdate(const meshtastic::Status *status);
    int onInputEvent(const InputEvent *event);
    std::string executeCommand(const std::string &command);
    void sendResponse(const meshtastic_MeshPacket &request, const std::string &response);
};
