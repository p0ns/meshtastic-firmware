...
inline bool matchShortCommand(const std::string &command, char alias,
                              size_t *argumentOffset = nullptr) {
  ...
  if (offset < command.size() &&
      !std::isspace(static_cast<unsigned char>(command[offset]))) {
    return false;
  }
...
inline bool isShortCommand(const std::string &command, char alias) {
  ...
  return matchShortCommand(command, alias, &argumentOffset) &&
         argumentOffset == command.size();
}
